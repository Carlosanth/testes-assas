...
(function Harvey(){
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Tema em tempo real ─────────────────────────────────────
    async function carregarTemaConfig() {
      const { data: dados, error } = await supabase
        .from("configuracoes")
        .select("modo_display, cor_primaria, tema, titulo_boas_vindas, descricao_boas_vindas, data_evento")
        .eq("usuario_id", usuarioIdUrl)
        .maybeSingle();
      if (error) { console.error("Erro tema:", error); return; }
      if (!dados) return;
      ...
      // cor_primaria (banco) = cor_tema (nome usado no restante deste arquivo)
      if (dados.cor_primaria) aplicarTema(dados.cor_primaria, modoForcado);

      // tema (banco) = imagem_fundo (nome usado no restante deste arquivo)
      if (dados.tema) {
        document.documentElement.style.setProperty(
          '--imagem-fundo', `url('${dados.tema}')`
        );
      }
    ...
    carregarTemaConfig();
    supabase
      .channel(`config-tema-${usuarioIdUrl}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'configuracoes', filter: `usuario_id=eq.${usuarioIdUrl}` }, carregarTemaConfig)
      .subscribe();
    ...
    // Cache local nome de categoria (id -> nome), montado a partir de
    // categorias (globais) + categorias_custom (privadas do dono da lista).
    // No Firestore o produto guardava o NOME direto; agora é categoria_id.
    ...
    async function buscarERenderizarProdutosPublico() {
       ...
       try {
        ...
        // cotas_ocupadas é uma tabela separada agora, não um array no produto
        const idsComCota = produtos.filter(p => p.cotas_total >= 2).map(p => p.id);
        const ocupadasPorProduto = {};
        if (idsComCota.length) {
          const { data: ocupadas } = await supabase
            .from("cotas_ocupadas")
            .select("produto_id, numero")
            .in("produto_id", idsComCota);
          (ocupadas || []).forEach(o => {
            if (!ocupadasPorProduto[o.produto_id]) ocupadasPorProduto[o.produto_id] = [];
            ocupadasPorProduto[o.produto_id].push(o.numero);
          });
        }
        ...
        produtos.forEach((produto) => {
          ...
          const nomeCategoria = produto.categoria_id ? (nomesCategoriaCache[produto.categoria_id] || '') : '';
          todosProdutosCache.push({ id, ...produto, categoria: nomeCategoria });

          if (nomeCategoria && nomeCategoria.trim()) {
            categoriasSet.add(nomeCategoria.trim());
          }
          ...
          wrap.dataset.categoria = nomeCategoria.trim() || '';
          ...
          img.src = produto.imagem_url || 'https://i.ibb.co/YBZJdZ2N/icon-192.jpg';
          ...
          const precoFmt = (precoCentavos / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
          ...
          caixaPreco.querySelector('.preco').textContent          = precoFmt || 'Consulte';
          ...
          const cotasOcupadas  = ocupadasPorProduto[id] || [];
    ...
    (async () => {
      await carregarNomesCategoria();
      await buscarERenderizarProdutosPublico();
    })();

    supabase
      .channel(`presentes-publico-${usuarioIdUrl}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'presentes', filter: `usuario_id=eq.${usuarioIdUrl}` }, buscarERenderizarProdutosPublico)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'cotas_ocupadas' }, buscarERenderizarProdutosPublico)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'categorias' }, carregarNomesCategoria)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'categorias_custom', filter: `usuario_id=eq.${usuarioIdUrl}` }, carregarNomesCategoria)
      .subscribe();
    ...
    function abrirModalEscolha(titulo, cotasTotal, cotasDisp, precoCentavos, cotasOcupadas) {
      ...
      document.getElementById('btn-escolha-tudo')?.addEventListener('click', () => {
        // "Presentear tudo" num item com cotas não é um presente "sem cota" —
        // o backend sempre trata cotas_total > 1 como fluxo de cota. Aqui a
        // gente monta a lista de TODOS os números ainda disponíveis e manda
        // como se o convidado tivesse selecionado cada um deles.
        const ocupadasSet = new Set(cotasOcupadas || []);
        const todasDisponiveis = [];
        for (let n = 1; n <= cotasTotal; n++) {
          if (!ocupadasSet.has(n)) todasDisponiveis.push(n);
        }
        modoFluxo = "cota";
        cotasEscolhidas = todasDisponiveis.length;
        cotasNumerosEscolhidos = todasDisponiveis;
        fechar();
        abrirModalNome();
      });
    ...
    async function confirmarNome() {
      ...
      if (!email) { alert("Digite seu e-mail para continuar."); return; }
      fecharModalNome();
      await finalizarCompra(nome, email);
    }
    ...
    async function finalizarCompra(nomeConvidado, emailConvidado) {
      // Mostra a telinha de transição já aqui — a chamada pra Edge Function
      // (que cria o checkout no Asaas) pode levar alguns segundos antes do
      // redirecionamento.
      ...
      try {
        ...
        // Se for fluxo de cota, envia os números exatos escolhidos — a Edge
        // Function usa esses números pra reservar exatamente essas cotas,
        // evitando conflito se dois convidados confirmarem juntos.
        if (modoFluxo === "cota" && cotasEscolhidas > 0) {
          payload.cotas_numeros = cotasNumerosEscolhidos;
        }

        const res = await fetch(edgeFunctions.finalizarCompra, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
        const resultado = await res.json();
        if (res.ok && resultado?.checkoutUrl) {
          // Mantém o overlay visível — a página vai navegar pra fora agora.
          window.location.href = resultado.checkoutUrl;
        } else {
          esconderOverlayRedirecionando();
          const mensagens = {
            presente_nao_encontrado: "Este presente não foi encontrado.",
            presente_indisponivel: "Este presente não está mais disponível.",
            presente_ja_confirmado: "Alguém já confirmou este presente.",
            presente_reservado: "Este presente está reservado no momento. Tente novamente em alguns minutos.",
            cota_indisponivel: "Uma ou mais cotas escolhidas já foram reservadas por outra pessoa. Atualize a página e tente de novo.",
            nenhuma_cota_informada: "Não foi possível identificar as cotas escolhidas. Tente novamente.",
            cadastro_incompleto: "O dono da lista ainda não completou o cadastro. Tente novamente mais tarde.",
          };
          const chaveErro = resultado.erro || 'erro_desconhecido';
          alert(mensagens[chaveErro] || "Não foi possível gerar o link. Tente novamente.");
        }
      } catch (err) {
